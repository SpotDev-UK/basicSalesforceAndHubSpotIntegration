...
const sfdcContactOrLead = "contact_or_account";
...
if (trigger.type === "Contact") {
    ...
    console.error(`Trigger received Record ID ${trigger.ID} that was not a Contact, Account or Lead`);
}

async function receivedSalesforceContact(salesforceRecord, objectType, propertyMappings) {
    const hsProperties = {
        "properties": {
            "email": salesforceRecord.email,
            [sfdcIdPropertiesInHubSpot[objectType]]: salesforceRecord.ID
        }
    };

    // Map any other fields
    mapSalesforceToHubSpotProperties(salesforceRecord, hsProperties.properties, propertyMappings);

    // This is an example. In production, make sure you're upserting or handling lookup logic as necessary
    try {
        await axios.post('https://api.hubapi.com/crm/v3/objects/contacts', hsProperties, {
            headers: {
                Authorization: `Bearer ${process.env.HUBSPOT_TOKEN}`,
                'Content-Type': 'application/json'
            }
        });
        console.log(`HubSpot contact upserted for Salesforce ${objectType} ID: ${salesforceRecord.ID}`);
    } catch (err) {
        console.error(`Error upserting HubSpot contact: ${err.message}`);
    }
}

async function receivedSalesforceAccount(salesforceRecord, objectType, propertyMappings) {
    ...
    // This is an example. In production, make sure you're upserting or handling lookup logic as necessary
    try {
        await axios.post('https://api.hubapi.com/crm/v3/objects/companies', hsProperties, {
            headers: {
                Authorization: `Bearer ${process.env.HUBSPOT_TOKEN}`,
                'Content-Type': 'application/json'
            }
        });
        console.log(`HubSpot company upserted for Salesforce ${objectType} ID: ${salesforceRecord.ID}`);
    } catch (err) {
        console.error(`Error upserting HubSpot company: ${err.message}`);
    }
}

function receivedSalesforceLead(salesforceRecord) {
    const contactOrAccountValue = salesforceRecord[sfdcContactOrLead];

    if (contactOrAccountValue === "Contact") {
        // Treat as a Contact
        receivedSalesforceContact(salesforceRecord, "contact", leadAsContactMappings);
    } else if (contactOrAccountValue === "Account") {
        // Treat as an Account
        receivedSalesforceAccount(salesforceRecord, "account", leadAsAccountMappings);
    } else {
        console.error(`New Salesforce Lead (ID: ${salesforceRecord.ID}) that was neither a Contact nor Account`);
    }
}

// Maps Salesforce fields to HubSpot fields using the provided mapping object
function mapSalesforceToHubSpotProperties(salesforceRecord, hsPropertyObject, mappingObject) {
...
// e.g.:
// "https://www.spotdev.co.uk" -> "spotdev.co.uk"
// "app.facebook.com" -> "facebook.com"
// "blog.spotdev.co.uk" -> "spotdev.co.uk"
